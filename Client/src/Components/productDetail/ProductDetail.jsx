...
class ProductDetail extends React.Component {
  ...
  getStyles() {
    // fetches all styles for currently selected product
    return axios({
      method: "get",
  ...
  componentDidMount() {
    // updates state with available styles and initializes to default style
    Promise.all([this.getProduct(), this.getStyles()]).then((response) => {
      let selected = {};
  ...
  handleChangeStyle(e) {
    // updates selected style when thumbnail is clicked
    ...
    return (
      <div onClick={incrementCount} id="ProductDetail">
        {this.state.expandedView ? (
          ...
          <div>
            <ImageGallery
              ...
              renderExpandedView={this.renderExpandedView}
        ...
        <div id="product_info" className="productDetailTier2">
          <Rating />
          <h3 className="product_category">{this.state.product.category}</h3>
          <h2 className="product_title">{this.state.product.name}</h2>
          {this.price()}
          <br />
          <span>Style: {this.state.selectedStyle.name}</span>
