...
function ProductDisplay() {
  ...
  const [comparisonPopupState, setComparisonPopupState] = useState({
    open: false,
  });
  ...
  const productAPI =
    "https://app-hrsei-api.herokuapp.com/api/fec2/hr-lax/products/";
  const reviewsAPI =
    "https://app-hrsei-api.herokuapp.com/api/fec2/hr-lax/reviews/meta";
  const { mainProduct, product, isOutfit, removeItem } =
  ...
  useEffect(() => {
    axios
      ...
      .then(() => {
        axios
          .get(`${reviewsAPI}?product_id=${product}`, {
            headers: { Authorization: `${TOKEN}` },
          })
          .then((results) => {
  ...
  const renderCloseButton = () => {
    return (
      <div>
        <FaRegTimesCircle
          className="close-outfit-display"
          onClick={removeProductDisplay}
        />
      </div>
    );
  };
  ...
  return (
    <div className="product-display" onClick={onProductDisplayClicked}>
      {image === null ? (
        <img className="product-image" src={imageplaceholder} />
      ) : (
      ...
      {comparisonPopupState.open === true && (
        <ComparisonPopup
          state={comparisonPopupState}
          setComparisonState={setComparisonPopupState}
        />
      )}
    </div>
  );
